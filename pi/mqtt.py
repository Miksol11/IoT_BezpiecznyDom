...
BROKER_HOST = "localhost" #10.108.33.106
BROKER_PORT = 1883
...
def add_card(card_id):
    res = _call(REQ_ADD_TOPIC, RES_ADD_TOPIC, (card_id or "").strip(), 5)
    return res == "true"


def delete_card(card_id):
    res = _call(REQ_DEL_TOPIC, RES_DEL_TOPIC, (card_id or "").strip(), 5)
    return res == "true"


def card_exists(card_id):
    res = _call(REQ_GET_TOPIC, RES_GET_TOPIC, (card_id or "").strip(), 5)
    return res == "true"

def list_cards():
    res = _call(REQ_ALL_TOPIC, RES_ALL_TOPIC, "", 5)
    if not res:
        return []
